...
| `onscreen_wifi_setup.md` | **Not started.** Phase 0 (scan still returns 0 networks in setup mode, v1.9.2) must land before Phase 1 (encoder-only network picker + full keyboard). Screen build only. |
...
| `flight_club_spec.md` | **In progress — Phase 1 built, not yet released or hardware-tested.** Flight Club: multi-participant sequential flights with a held-back cumulative reveal, fully operable from the encoder with no phone. Prerequisite disc-ownership refactor is **v1.6.2** (`motorDisable()` clears position trust; `motorPositionIsVerified()` replaces the per-module `homedThisFlight` bools). Phase 1 is **v1.7.0**: `party.cpp` engine, `roster.cpp`, rank scoring with tiebreak and unanimous flag, paged reveal, own NVS blob with resume. Phase 2 = guess variants. Phase 3 = participant phones, needing role separation in `wifi_portal.cpp`. |
